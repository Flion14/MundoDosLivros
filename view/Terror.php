...
<?php
	if(isset($_GET['pesq']) && $_GET['pesq'] != ""){
		$pes = $_GET['pesq'];
		$consulta = "select img, nome, preco, cod_genero FROM livro left join genero_livro on livro.cod_livro=genero_livro.cod_livro where cod_genero like 3 and nome like '%$pes%';";
	}else{
		$consulta = "select img, nome, preco, cod_genero FROM livro left join genero_livro on livro.cod_livro=genero_livro.cod_livro where cod_genero like 3;";
	}
